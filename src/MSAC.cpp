...
/*    __K.at<float>(0,0) = fSettings["Camera.fx"];
    ...
    __K.at<float>(0,0) = 517.306408;
    __K.at<float>(0,2) = 318.643040;
    __K.at<float>(1,1) = 516.469215;
    __K.at<float>(1,2) = 255.313989;
    __K.at<float>(2,2) = (float)1;
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	colors.push_back(cv::Scalar(128,0,128)); // Fouth is PURPLE
	colors.push_back(cv::Scalar(255,255,0)); // Fifth is YELLOW
	...
	// Paint line segments
	int colorSize;
	colorSize = (lineSegmentsClusters.size() < colors.size())?lineSegmentsClusters.size():colors.size();
	for(unsigned int c=0; c<colorSize; c++)
	{
