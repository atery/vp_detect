...
void MSAC::init(int mode, cv::Size imSize, bool verbose)
{
    ...
    __T_noise_squared = (float)0.001623*2;
	__min_iters = 5;
    //__max_iters = INT_MAX;
    __max_iters = 100000;
	...
	__K = Mat(3,3,CV_32F);
	__K.setTo(0);
    __K.at<float>(0,0) = fSettings["Camera.fx"];
    __K.at<float>(0,2) = fSettings["Camera.cx"];
    __K.at<float>(1,1) = fSettings["Camera.fy"];
    __K.at<float>(1,2) = fSettings["Camera.cy"];
...
#define theta_simi_threshold 8*CV_PI/180
#define theta_orth_threshold 82*CV_PI/180
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	for(unsigned int vpNum=0; vpNum<vps.size(); vpNum++)
	{
		if(vps[vpNum].at<float>(2,0) != 0)
		{
			...
			{
				Point pt3,pt4;
				float dist=sqrt(vp.x*vp.x+vp.y*vp.y);
				pt3.x=376;pt3.y=240;
				pt4.x=(int)(100*vp.x/dist);
                pt4.y=(int)(100*vp.y/dist);
				line(im, pt3, pt4, colors[vpNum], 1);
            }*/
		}
        printf("vps.size=%d\n lineSegmentsClusters.size =%d\n",vps.size(),lineSegmentsClusters.size());
	}
	// Paint line segments

    vector<double> thetas;
    vector<Point> points;
    static double last_theta[2]={0.0,0.0};
	for(unsigned int c=0; c<lineSegmentsClusters.size(); c++)
	{
        if(vps.size() > c)
        {
            Point2f vp2(vps[c].at<float>(0,0), vps[c].at<float>(1,0));
            double theta=0.0,theta_sum=0.0;
            int theta_cnt=0,theta_cnt2=0;
            ...
            for(unsigned int i=0; i<lineSegmentsClusters[c].size(); i++)
            {
                ...
                //if(fabs(vp2.x) > im.cols || fabs(vp2.y) > im.rows)
                if(vp2.x*vp2.x + vp2.y*vp2.y > 10000*10000)
                {
                    if(pt1.x < pt2.x){theta=atan2(pt2.y-pt1.y,pt2.x-pt1.x);theta_sum+=theta;theta_cnt++;}
                    else if(pt1.x > pt2.x){theta=atan2(pt1.y-pt2.y,pt1.x-pt2.x);theta_sum+=theta;theta_cnt++;}
                    else
                    {
                        theta_cnt2++;
                    }
            ...
            if(theta_cnt || theta_cnt2)
            {
                if(theta_cnt)theta=theta_sum/theta_cnt;
                else if(theta_cnt2)theta= CV_PI/2;
                thetas.push_back(theta);
                Point pt3,pt4;
                pt3.x=376;pt3.y=240;
                pt4.x=376+200*cos(theta);
                pt4.y=240+200*sin(theta);
                points.push_back(pt3);
                points.push_back(pt4);
            }
        }
    }
    //if(thetas.size() > 0)line(im, points[0], points[1], colors[0], 4);
    if((0.0 ==last_theta[0]) && (0.0 == last_theta[1]))
    {
        if(thetas.size() > 1)
        {
            double cos_tmp=1.0;
            ...
            for(unsigned int i=0;i < thetas.size();i++)
            {
                for(unsigned int j=0;j < thetas.size();j++)
                {
                        if(j != i)
                        {
                            if(fabs(cos(thetas[i]-thetas[j])) < cos_tmp)
                            {
                              cos_tmp= fabs(cos(thetas[i]-thetas[j]));
                              tmp_i=i;tmp_j=j;
                            }
                        }
                }
            }
            if(cos_tmp < cos(theta_orth_threshold))
            {
              last_theta[0]=thetas[tmp_i];
              last_theta[1]=thetas[tmp_j];
            }
            else
            {
                last_theta[0]=thetas[0];
                last_theta[1]=thetas[0]+ CV_PI/2;
            }
        }
        else if(1 == thetas.size())
        {
            last_theta[0]=thetas[0];
            last_theta[1]=thetas[0]+ CV_PI/2;
        }
        Point Pt1,Pt2,Pt3;
        Pt1.x=376;Pt1.y=240;
        Pt2.x=376+200*cos(last_theta[0]);
        Pt2.y=240+200*sin(last_theta[0]);
        Pt3.x=376+200*cos(last_theta[1]);
        Pt3.y=240+200*sin(last_theta[1]);
        line(im, Pt1, Pt2, colors[0], 4);
        line(im, Pt1, Pt3, colors[1], 4);
        /*
        if(thetas.size()<3)
        {
            for(unsigned int i=0;i < thetas.size();i++)line(im, points[2*i], points[2*i+1], colors[i], 4);
        }
        else if(thetas.size() == 3)
        {
            line(im, points[0], points[1], colors[0], 4);
            if(cos(thetas[2]-thetas[0]) >= cos(thetas[1]-thetas[0]))line(im, points[2], points[3], colors[1], 4);
            else line(im, points[4], points[5], colors[1], 4);
        }
        */
    }
    else
    {
        ...
        {
            double theta_temp[2];
            theta_temp[0]=last_theta[0];theta_temp[1]=last_theta[1];
            for(unsigned int i=0;i < 2;i++)
            {
                if(cos(last_theta[i]-thetas[0]) > cos(theta_simi_threshold))
                {
                    theta_temp[i]=thetas[0];
                    if(last_theta[1-i] > last_theta[i])theta_temp[1-i]=thetas[0]+ CV_PI/2;
                    else theta_temp[1-i]=thetas[0] - CV_PI/2;
                }
            }
            Point Pt1,Pt2,Pt3;
            Pt1.x=376;Pt1.y=240;
            Pt2.x=376+200*cos(theta_temp[0]);
            Pt2.y=240+200*sin(theta_temp[0]);
            Pt3.x=376+200*cos(theta_temp[1]);
            Pt3.y=240+200*sin(theta_temp[1]);
            line(im, Pt1, Pt2, colors[0], 4);
            line(im, Pt1, Pt3, colors[1], 4);
            last_theta[0]=theta_temp[0];
            last_theta[1]=theta_temp[1];
        }
        else
        {
            double cos_tmp=1.0;
            ...
            if(cos_tmp < cos(theta_orth_threshold))
            {
                if(last_theta[0] > last_theta[1])
                {
                    last_theta[0]=(thetas[tmp_i] > thetas[tmp_j])?thetas[tmp_i]:thetas[tmp_j];
                    last_theta[1]=(thetas[tmp_i] > thetas[tmp_j])?thetas[tmp_j]:thetas[tmp_i];
                }
                else
                {
                    last_theta[1]=(thetas[tmp_i] > thetas[tmp_j])?thetas[tmp_i]:thetas[tmp_j];
                    last_theta[0]=(thetas[tmp_i] > thetas[tmp_j])?thetas[tmp_j]:thetas[tmp_i];
                }
                Point Pt1,Pt2,Pt3;
                Pt1.x=376;Pt1.y=240;
                Pt2.x=376+200*cos(last_theta[0]);
            ...
            {
                double cos_tmp2=0.0;
                double last_theta_tmp[2]={0.0,0.0};
                int tmp2_i=0,tmp2_cnt=0;
                last_theta_tmp[0]=last_theta[0];
                last_theta_tmp[1]=last_theta[1];
                for(unsigned int i=0;i < thetas.size();i++)
                {
                    if((fabs(cos(thetas[i]-last_theta[0])) > cos_tmp2)||fabs(cos(thetas[i]-last_theta[1])) > cos_tmp2)
                    {
                      cos_tmp2= (fabs(cos(thetas[i]-last_theta[0])) > fabs(cos(thetas[i]-last_theta[1])))?fabs(cos(thetas[i]-last_theta[0])):fabs(cos(thetas[i]-last_theta[1]));
                      tmp2_cnt=(fabs(cos(thetas[i]-last_theta[0])) > fabs(cos(thetas[i]-last_theta[1])))?0:1;
                      tmp2_i=i;
                    }
                }
                if(cos_tmp2 > cos(theta_simi_threshold))
                {
                    last_theta_tmp[tmp2_cnt]=thetas[tmp2_i];
                    if(last_theta[1-tmp2_cnt] > last_theta[tmp2_cnt])last_theta_tmp[1-tmp2_cnt]=thetas[tmp2_i]+ CV_PI/2;
                    else last_theta_tmp[1-tmp2_cnt]=thetas[tmp2_i] - CV_PI/2;
                }
                else
                {
                    last_theta_tmp[tmp2_cnt]=(thetas[tmp2_i] + last_theta[tmp2_cnt]) / 2.0;
                    if(last_theta[1-tmp2_cnt] > last_theta[tmp2_cnt])last_theta_tmp[1-tmp2_cnt]=last_theta_tmp[tmp2_cnt] + CV_PI/2;
                    else last_theta_tmp[1-tmp2_cnt]=last_theta_tmp[tmp2_cnt] - CV_PI/2;
                }

                last_theta[0]=last_theta_tmp[0];
                last_theta[1]=last_theta_tmp[1];
                Point Pt1,Pt2,Pt3;
                Pt1.x=376;Pt1.y=240;
                Pt2.x=376+200*cos(last_theta[0]);
                Pt2.y=240+200*sin(last_theta[0]);
